...
FROM consoles c,
LATERAL jsonb_array_elements(COALESCE(c.urls, '[]'::jsonb)) WITH ORDINALITY AS t(url_item, ordinality)
...
FROM emulators e,
LATERAL jsonb_array_elements(COALESCE(e.urls, '[]'::jsonb)) WITH ORDINALITY AS t(url_item, ordinality)
WHERE (url_item->>'url') IS NOT NULL
ON CONFLICT DO NOTHING;
...
FROM games g,
LATERAL jsonb_array_elements(COALESCE(g.urls, '[]'::jsonb)) WITH ORDINALITY AS t(url_item, ordinality)
WHERE (url_item->>'url') IS NOT NULL
ON CONFLICT DO NOTHING;
...
FROM tools t,
LATERAL jsonb_array_elements(COALESCE(t.urls, '[]'::jsonb)) WITH ORDINALITY AS t(url_item, ordinality)
WHERE (url_item->>'url') IS NOT NULL
ON CONFLICT DO NOTHING;
